 ...
 * 图片尺寸为宽度铺满高度自适应
 */
public class ImageViewHeight extends AppCompatImageView {

    float width, height;
    int radian = 8; //圆角大小

    public ImageViewHeight(Context context) {
        this(context, null);
        init(context, null);
    }

    public ImageViewHeight(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
        init(context, attrs);
    }

    public ImageViewHeight(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (d != null) {
            //宽度铺满整个屏幕， 高度自适应
            int widthPix = MeasureSpec.getSize(widthMeasureSpec);
            //高度根据使得图片的宽度充满屏幕计算而得
